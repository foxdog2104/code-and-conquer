...
=======
signal level_won
@export var enemy_path_path: NodePath
@onready var enemyPath: Path2D = get_node(enemy_path_path) as Path2D
...
var waves := [
	...
	[
		{ "enemy": ENEMY_OGRE, "count": 7, "interval": 0.1 },
	],
]
...
func _check_win_condition() -> void:
	...
	var last_wave_done := (current_wave_index == waves.size() - 1) and (active_wave == false)
	var no_enemies_left := enemyPath.get_child_count() == 0

	print("win check | wave:", current_wave_index, "/", waves.size()-1,
		" active:", active_wave,
		" children:", enemyPath.get_child_count(),
		" last_wave_done:", last_wave_done,
		" no_enemies_left:", no_enemies_left)

	if last_wave_done and no_enemies_left:
		_won_emitted = true
		print("EMITTING level_won")
		level_won.emit()
